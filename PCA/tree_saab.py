...
def PixelHop_Neighbour(feature, dilate, window_size, pad):
    ...
    print("   <Neighborhood Info>filter size:", str(2 * window_size + 1), str(2 * window_size + 1),
          str(feature.shape[-1]))
...
def decide_pass_idx1(feature, leaf_energy_last, Leaf_Ener_thrs, Pass_Ener_thrs, hopidx=1):
    ...
    plt.savefig(
        '/mnt/yaozhu/image_splicing_mnt/Output_Mat_Files/regression_v1/channelwisepca/plots/variance_of_hop' + str(
            hopidx - 1) + '_sorted.png')
    plt.close(0)
