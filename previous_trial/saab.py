...
def find_kernels_pca(sample_patches, num_kernels, energy_percent):
	...
	pca=PCA(n_components=training_data.shape[1], whiten=True, svd_solver='full')
...
def multi_Saab_transform(images, labels, kernel_sizes, num_kernels, energy_percent, use_num_images, use_classes):
	...
	for i in range(num_layers):
		...
		h=sample_patches.shape[1] #8
		w=sample_patches.shape[2] #8
    	# Flatten
		sample_patches=sample_patches.reshape([-1, sample_patches.shape[-1]]) #n*8*8, 3*4*4
