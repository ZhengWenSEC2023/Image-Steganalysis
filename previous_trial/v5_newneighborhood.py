...
def cross_entropy(data_pca, labels):
    # GC will be labels

    # print("transformed data dimension: ", data_pca.shape)
    # print("label dimension: ", labels.shape)

    clf = RandomForestClassifier(n_estimators=10)
    ce = np.zeros((data_pca.shape[-1]))

    for i in range(data_pca.shape[-1]):
        data_pca_slice = data_pca[:,i].reshape((data_pca.shape[0], 1))
        clf.fit(data_pca_slice, labels)
        predicted_prob = clf.predict_proba(data_pca_slice)
        ce[i] = LL(labels, predicted_prob)

    return ce

def residual_learning(attri_saab, GC):

    # 256, 284, 75(25)
    print("input shape for residual learning: ", attri_saab.shape) # 5,256,384,75

    # reshape spacial
    attri_saab_flatten = attri_saab.reshape((-1, attri_saab.shape[-1]))
    data = attri_saab_flatten
    ...
    patch_mean = np.mean(data, axis=1, keepdims=True)  # dc
    data = data - patch_mean

    # data = saab.remove_zero_patch(data)
    # print("seperate DC component:", data.shape)

    feature_mean = np.mean(data, axis=0, keepdims=True)
    data = data - feature_mean

    k = data.shape[-1]
    pca = PCA(n_components=k, svd_solver='full', whiten=True)
    data_transform = pca.fit_transform(data)

    # calculate cross entropy for all attributes, select the one with lowest ce value
    labels = GC.flatten()
    ce = cross_entropy(data_transform, labels)
    order = np.argsort(ce)
    print(order)
    ...
    for k in range(27):
        ...
        # idx = np.argmin(ce)
        print("location for minimum ce attribute in each level:", idx)
        idxs.append(idx)

        pri = data_transform[:, idx]
        # bias = LA.norm(data, axis=1)
        # bias = np.max(bias)
        # pri = pri + bias
        primary.append(pri)
        AC_kernel.append(pca.components_[idx])

        # data_transform[:, idx] = 0
        # residual = data_transform
        # data = pca.inverse_transform(residual)

        # if sum(eigenvalues)> 0.95:
        #     print(attri_saab_flatten.shape[-1] - k)
        #     break

    primary = np.array(primary)  # 5*256*384, k
    primary = primary.transpose()
    # print("total primary shape:", primary.shape)

    AC_kernel = np.array(AC_kernel)
    # AC_kernel = AC_kernel.transpose()
    print("AC kernel shape:", AC_kernel.shape)


    return AC_kernel, primary, idxs
...
def one_hop(extended_imgs):
    ...
    AC_kernel, primary, idxs = residual_learning(one_hop_attri_saab, GC)
    one_hop_attri_reshape = one_hop_attri.reshape((-1, one_hop_attri.shape[-1]))

    print(pca_params_1['Layer_0/kernel'].shape, AC_kernel.shape)
    print(one_hop_attri_reshape.shape, primary.shape)


    # print(one_hop_attri_reshape - primary)
    for i in range(len(idxs)):
        print(pca_params_1['Layer_0/kernel'][idxs[i]] - AC_kernel[i])


    # print("one hop attribute shape:", one_hop_attri.shape)


    return pca_params_1, AC_kernel, one_hop_attri, primary
...
#    test_dir = '/Users/mac/Desktop/mcl_2019/image_splicing/example_files/COLUMBIA/'
    ...
    save_dir_matFiles = '/mnt/yaozhu/image_splicing_mnt/Output_Mat_Files/Neighbor25_100_stride3/'
...
#%%
    # start = time.time()
    # pca_params_1, AC_kernel = one_hop(ext_train_imgs)
...
#%%
    ...
    #
    # # for k in range(3):
    # #     cv2.imwrite(save_dir_matFiles + 'pos0sample/ori_%d.png' % k, ext_train_imgs[k])
    # #     cv2.imwrite(save_dir_matFiles + 'pos0sample/dilated_%d.png' % k, boundary[k])
    # #     cv2.imwrite(save_dir_matFiles + 'pos0sample/authen_%d.png' % k, authentic_edges[k])
    #
...
#%%
   ...
   #
    ...
    #     for i in a:
    #         for j in b:
    #             if (authentic_edges[k,i,j] !=0) and (t <= int(counts_pos[k]*1.5)):
    #                 t = t+1
    ...
    # cPickle.dump(negative2, open(save_dir_matFiles+'neg_2_100_15times.pkl', 'wb'))
...
# %%
    ...
    # positive3 = np.load(save_dir_matFiles + 'pos_3_100_pool.npy')
    #
    negative1 = cPickle.load(open(save_dir_matFiles + 'neg_1_100.pkl', 'rb'))
    negative2 = cPickle.load(open(save_dir_matFiles + 'neg_2_100_15times.pkl', 'rb'))
    # negative3 = cPickle.load(open(save_dir_matFiles + 'neg_3_100_pool.pkl', 'rb'))

    print("spliced and natural number:", positive2.shape[0], negative2.shape[0])
    ...
    pos_label = np.ones((positive2.shape[0],), dtype=int)
    neg_label = np.zeros((negative2.shape[0],), dtype=int)
...
#%%
    ...
    # test_positive3 = np.load(save_dir_matFiles + 'test_pos_3_100_pool.npy')
    # test_negative3 = np.load(save_dir_matFiles + 'test_neg_3_100_pool.npy')
    ...
    print("test spliced and natural number:", test_positive1.shape[0], test_negative1.shape[0])
...
# %%
    ...
    # clf1.fit(train_feature, train_labels)
    # predicted_train_label = clf1.predict(train_feature)
    #
    ...
    #
    #
    # test_predicted_label = clf1.predict(test_feature)
    ...
    class_frequency = np.array((positive1.shape[0], negative1.shape[0]))

    # it is a LIST!!!
    train_feature_reduce = LAG_Unit(train_feature, train_labels=train_labels, class_list=class_list,
                                    class_frequency=class_frequency, SAVE=SAVE, num_clusters=num_clusters,
                                    alpha=15, Train=True)
    print(SAVE.keys())
    ...
    # it is a LIST!!!
    test_feature_reduce = LAG_Unit(test_feature, train_labels=None, class_list=None, SAVE=SAVE, num_clusters=None,
                                   alpha=None, Train=False)
    # print(test_feature_reduce.shape)

    for k in range(int(len(SAVE)/4)-1):
        print("----feature----:", k)
        print("feed in classifier shape:", train_feature_reduce[k].shape)
        clf1.fit(train_feature_reduce[k], train_labels)

        predicted_train_label = clf1.predict(train_feature_reduce[k])
        ...
        test_predicted_label = clf1.predict(test_feature_reduce[k])
        C_test = confusion_matrix(test_labels, test_predicted_label)
        per_class_accuracy_test = np.diag(C_test.astype(np.float32)) / np.sum(C_test.astype(np.float32), axis=1)
        print(per_class_accuracy_test)
