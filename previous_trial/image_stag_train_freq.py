...
if __name__ == "__main__":
    ...
    test_imgs = readImgToNumpy('/mnt/zhengwen/image_steganalysis/test_img')
    ...
    start = time.time()
    # Hop-1
    high_frez_response, low_frez_response, high_energy, low_energy, test_context_hop2_input, test_context_hop1 = one_hop(imgs, test_imgs)
    high_frez_response = np.squeeze(high_frez_response)
    low_frez_response = np.squeeze(low_frez_response)
    test_context_hop1 = np.squeeze(test_context_hop1)
    test_context_hop2_input = np.squeeze(test_context_hop2_input)
    print("PixelHop 1 output", low_frez_response.shape)

    # sec_hop_input = block_reduce(high_frez_response, block_size=(1, 2, 2, 1), func=np.max)
    sec_hop_input = block_reduce(high_frez_response, block_size=(1, 2, 2, 1), func=np.max)
    test_context_hop2_input = block_reduce(test_context_hop2_input, block_size=(1, 2, 2, 1), func=np.max)
    # Hop-2
    sec_hop_attri_response, sec_hop_engr, sec_test_context = sec_hop(sec_hop_input, high_energy, test_context_hop2_input)
    print("PixelHop 2 output", sec_hop_attri_response.shape)

    print("Begin to save!")

    print(low_frez_response.shape)
    print(high_frez_response.shape)
    print(sec_hop_attri_response.shape)
    print(test_context_hop1.shape)
    print(test_context_hop2_input.shape)
    print(sec_test_context.shape)

    np.save("low_freq_hop1.npy", low_frez_response)
    np.save("high_freq_hop1.npy", high_frez_response)
    np.save("high_low_freq_hop2.npy", sec_hop_attri_response)

    np.save("low_freq_hop1_test.npy", test_context_hop1)
    np.save("high_freq_hop1_test.npy", test_context_hop2_input)
