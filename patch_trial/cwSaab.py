...
#
import numpy as np 
from patch_trial.saab import Saab
...
class cwSaab():
    ...
    def cwSaab_n_layer(self, X, train, layer, reverse):
...
#        DC.append(dc)
        for i in range(1, self.depth):
            X, dc = self.cwSaab_n_layer(X, train=True, layer=i, reverse=False)
#            output.append(X)
#            DC.append(dc)
            if self.split == False:
                self.depth = i
...
#        return output, DC
    ...
    def transform(self, X):
        ...
        DC.append(dc)
        for i in range(1, self.depth):
            X, dc = self.cwSaab_n_layer(X, train=False, layer=i, reverse=False)
            output.append(X)
            DC.append(dc)
