...
pos_sample_test = np.concatenate(
    (np.squeeze(np.load(r"/mnt/zhengwen/new_trial/test_ori_ori_output_2.npy")),
     np.squeeze(np.load(r"/mnt/zhengwen/new_trial/test_ori_ste_output_2.npy"))), axis=1)

neg_sample_test = np.concatenate(
    (np.squeeze(np.load(r"/mnt/zhengwen/new_trial/test_ste_ori_output_2.npy")),
     np.squeeze(np.load(r"/mnt/zhengwen/new_trial/test_ste_ste_output_2.npy"))), axis=1)
...
f = open('/mnt/zhengwen/new_trial/RMclf_p2.pkl', 'rb')
clf = pickle.load(f)
f.close()

print(clf.score(pos_sample_test, label_pos))