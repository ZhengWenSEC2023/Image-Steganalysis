...
# UNIFORMLY
test_unchanged_ori_context = test_ori_context.copy()[diff_map == 0]
test_unchanged_steg_context = test_steg_context.copy()[diff_map == 0]

idx = np.random.permutation(len(test_unchanged_ori_context))
test_unchanged_ori_context = test_unchanged_ori_context[idx][:NUM_UNIFORM]
idx = np.random.permutation(len(test_unchanged_steg_context))
test_unchanged_steg_context = test_unchanged_steg_context[idx][:NUM_UNIFORM]
...
for i in range(1, len(CHANNEL_RANGE)):
    print(i)
    test_ori_vec = test_unchanged_ori_context[:, CHANNEL_RANGE[i - 1]: CHANNEL_RANGE[i]]
    test_steg_vec = test_unchanged_steg_context[:, CHANNEL_RANGE[i - 1]: CHANNEL_RANGE[i]]
    clf = clf_list[i - 1]
