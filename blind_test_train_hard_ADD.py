...
for i in range(len(counts)):
    train_ori_vec = train_unchanged_ori_context_hard[:, counts[i][0]: counts[i][-1]]
    train_steg_vec = train_unchanged_steg_context_hard[:, counts[i][0]: counts[i][-1]]
    clf = clf_list[i]
...
print("HARD UNCHANGED ORI SCORE (COMPARED WITH TOTAL 0):", svm_best.score(steg_prob, 1 * np.ones(len(steg_prob))))
